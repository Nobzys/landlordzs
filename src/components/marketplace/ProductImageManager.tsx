...
import { addProductImage, removeProductImage, setPrimaryProductImage } from '@/lib/actions/marketplace'
...
export function ProductImageManager({ userId, productId, images: initialImages }: ProductImageManagerProps) {
  ...
  const handleFile = (file: File) => {
    ...
    startTransition(async () => {
      try {
        ...
        const result = await addProductImage(productId, url, isPrimary)
        if (result?.error) throw new Error(result.error)

        const id = (result as { data?: { id: string } }).data?.id
        if (id) setImages((prev) => [...prev, { id, url, is_primary: isPrimary }])
      } catch (err: unknown) {
          ...
          <div key={img.id} className="relative group rounded-lg overflow-hidden border">
            ...
            <div className="absolute inset-0 flex items-center justify-center gap-2 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity">
              {!img.is_primary && (
                <button
                  ...
                  onClick={() => startTransition(async () => {
                    const res = await setPrimaryProductImage(img.id, productId)
                    if (!res?.error) {
              ...
              <button
                ...
                disabled={isPending}
                onClick={() => startTransition(async () => {
                  const res = await removeProductImage(img.id, productId)
