...
export const metadata: Metadata = { title: 'Service Request Detail — Architect' }

interface ArchitectRequestPageProps {
...
export default async function ArchitectRequestDetailPage({ params }: ArchitectRequestPageProps) {
  ...
  // If not found or not open, check if the architect has an accepted contract for it
  if (!request) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        <Link href="/architect/requests" className="text-sm text-muted-foreground hover:text-foreground transition-colors">
          ← My Requests
        </Link>
        ...
        <div className="rounded-xl border bg-card p-6 space-y-3">
          ...
          {contract.status === 'active' && (
            <form action={async () => {
              ...
              revalidatePath(`/architect/requests/${id}`)
            }}>
  ...
  // Request is open — fetch the architect's own quotation (if any)
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
