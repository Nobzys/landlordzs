...
import { startUserPreview, endUserPreview } from '@/lib/actions/auth'
...
export default async function AdminUserPreviewPage({
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="rounded-xl border border-amber-300 bg-amber-50 dark:bg-amber-900/20 dark:border-amber-700 p-4 flex items-center justify-between gap-4 flex-wrap sticky top-2 z-10">
        <div className="flex items-center gap-3">
          <AlertTriangle className="h-5 w-5 text-amber-600 dark:text-amber-400 shrink-0" />
          <p className="text-sm font-semibold text-amber-800 dark:text-amber-300">
            You are viewing this account as an administrator — Read Only
          </p>
        </div>
        <form action={async () => {
          'use server'
          ...
          <Button type="submit" size="sm" variant="outline" className="border-amber-400 text-amber-800 hover:bg-amber-100 dark:border-amber-600 dark:text-amber-300">
            Exit Preview
          </Button>
        </form>
      </div>
      ...
      {hasRating && (
        <p className="text-xs text-muted-foreground inline-flex items-center gap-1.5">
          <Star className="h-3.5 w-3.5" /> Reviews and full history are available in the admin user detail page.
        </p>
      )}
