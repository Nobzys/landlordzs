...
export const metadata: Metadata = { title: 'Service Request Detail — Contractor' }

interface ContractorRequestPageProps {
...
export default async function ContractorRequestDetailPage({ params }: ContractorRequestPageProps) {
  ...
  // If not found or not open, check if the contractor has an accepted contract for it
  if (!request) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        <Link href="/contractor/requests" className="text-sm text-muted-foreground hover:text-foreground transition-colors">
          ← My Requests
        </Link>
        ...
        <div className="rounded-xl border bg-card p-6 space-y-3">
          ...
          {contract.status === 'active' && (
            <form action={async () => {
              ...
              revalidatePath(`/contractor/requests/${id}`)
            }}>
  ...
  // Request is open — show details and quotation form
  // Fetch the contractor's own quotation for this request (if any)
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
