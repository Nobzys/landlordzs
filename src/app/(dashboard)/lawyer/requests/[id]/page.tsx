...
export const metadata: Metadata = { title: 'Service Request Detail — Lawyer' }

interface LawyerRequestPageProps {
...
export default async function LawyerRequestDetailPage({ params }: LawyerRequestPageProps) {
  ...
  // If not found or not open, check if the lawyer has an accepted contract for it
  if (!request) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        <Link href="/lawyer/requests" className="text-sm text-muted-foreground hover:text-foreground transition-colors">
          ← My Requests
        </Link>
        ...
        <div className="rounded-xl border bg-card p-6 space-y-3">
          ...
          {contract.status === 'active' && (
            <form action={async () => {
              ...
              revalidatePath(`/lawyer/requests/${id}`)
            }}>
  ...
  // Request is open — fetch the lawyer's own quotation (if any)
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
