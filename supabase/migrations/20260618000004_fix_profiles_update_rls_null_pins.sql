-- Fix: profiles_update_own (20260618000003) pinned approved_at/approved_by/
-- rejected_at/rejected_by using plain `=`. Per SQL three-valued logic,
-- `NULL = NULL` evaluates to UNKNOWN, not TRUE, so the WITH CHECK clause
-- rejected every self-update (e.g. updateBasicProfile) for any user who has
-- never been approved or rejected — i.e. almost everyone, including all
-- buyers. Switch those four nullable-column pins to `IS NOT DISTINCT FROM`,
-- which treats NULL = NULL as a match. role/is_verified/is_premium stay on
